...
func main() {
	...
	reqs := []int{
		...
		750, 250, 100, 200, // 4th day: 1100 requests
	...
	for N < len(reqs) {
		daily = append(daily, reqs[0:N]) // append N elements from the beggining of the reqs slice
		reqs = reqs[N:]                  // move pointer + N elements
	}
	// ================================================
	// #3: Print the results
