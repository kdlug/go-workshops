...
func main() {
	...
	// print students by house name
	s, ok := students[house]
	if !ok {
		fmt.Printf("Sorry. I don't know anything about %q.", house)
		return
	}

	cloneStudents := make([]string, len(s))

	// or clone := append([]string(nil), students...)
	copy(cloneStudents, s)
	sort.Strings(cloneStudents)

	fmt.Printf("~~~ %s students ~~~ .\n", house)

	for _, student := range cloneStudents {
		fmt.Printf("\t+ %s\n", student)
	}
}
